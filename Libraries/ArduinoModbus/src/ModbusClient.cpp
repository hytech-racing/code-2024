...
{
  int hrw = modbus_set_slave(_mb, id);    // 0 which looks correct im tripping
  // Serial.println(hrw);    // ****************change back*****************

  if (modbus_write_register(_mb, address, value) < 0) {
    // if (_mb == NULL)
    //   Serial.println("ctx tripping");    // **************change back******************
    // Serial.println("ctr did not trip but sth else tripped");
    return 0;
  }
