...
#define MAX_ALLOWED_POWER 79000
...
//      max_rear_power = 36000.0;
  ...
  if (mc_setpoints_command[0].get_pos_torque_limit() > 0 && mc_setpoints_command[1].get_pos_torque_limit() > 0
    ...
    float shunt_current = 100;    // measured shunt current
    uint16_t pack_voltage = mc_energy[0].get_dc_bus_voltage(); // dc voltage
    float iv_power = shunt_current * pack_voltage; // TS electrical power (inverter + misc)
    const int32_t actual_power = mc_energy[0].get_actual_power() + mc_energy[1].get_actual_power() + 
                                 mc_energy[2].get_actual_power() + mc_energy[3].get_actual_power(); // inverter mechanical power
    float tot_eff = actual_power/iv_power; // inverter efficiency 98% + other loss
    const float front_torque_distribution = max_front_power / (max_front_power + max_rear_power);
    const float rear_torque_distribution = max_rear_power / (max_front_power + max_rear_power);
    // max_front_power = 21760.0;
    // max_rear_power = 41230.0;
    max_front_power = max((float) max_front_power, MAX_ALLOWED_POWER * tot_eff * front_torque_distribution);
    max_rear_power = max((float) max_rear_power, MAX_ALLOWED_POWER * tot_eff * rear_torque_distribution);
