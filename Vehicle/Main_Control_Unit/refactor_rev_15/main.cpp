...
void parse_telem_can_message(const CAN_message_t &RX_msg) {
  ...
  switch (rx_msg.id) {
    ...
    case ID_DASHBOARD_STATUS:
      ...
      break;
    case ID_SAB_CB:
      sab_cb.load(rx_msg.buf);
      prev_load_cell_readings[2] = mcu_load_cells.get_RL_load_cell();
      prev_load_cell_readings[3] = mcu_load_cells.get_RR_load_cell();
      mcu_load_cells.set_RL_load_cell(sab_cb.get_RL_load_cell());
      mcu_load_cells.set_RR_load_cell(sab_cb.get_RR_load_cell());
      mcu_potentiometers.set_pot3(sab_cb.get_pot3());
      mcu_potentiometers.set_pot4(sab_cb.get_pot4());
  }
}
...
inline void read_all_adc_channels(MCP3204* adc, uint16_t* buf) {
  ...
  buf[0] = adc->read(MCP3204::Channel::SINGLE_0);
  buf[1] = adc->read(MCP3204::Channel::SINGLE_1);
  buf[2] = adc->read(MCP3204::Channel::SINGLE_2);
  buf[3] = adc->read(MCP3204::Channel::SINGLE_3);
  SPI.endTransaction();
}

inline void read_all_adc_channels(MCP3208* adc, uint16_t* buf) { //doubt this will work since 3208 might be same class as 3204
  SPI.beginTransaction(settings);
  buf[0] = adc->read(MCP3208::Channel::SINGLE_0);
  buf[1] = adc->read(MCP3208::Channel::SINGLE_1);
  buf[2] = adc->read(MCP3208::Channel::SINGLE_2);
  buf[3] = adc->read(MCP3208::Channel::SINGLE_3);
  buf[4] = adc->read(MCP3208::Channel::SINGLE_0);
  buf[5] = adc->read(MCP3208::Channel::SINGLE_1);
  buf[6] = adc->read(MCP3208::Channel::SINGLE_2);
  buf[7] = adc->read(MCP3208::Channel::SINGLE_3);
  SPI.endTransaction();
}
...
inline void read_all_adcs() {
  if (timer_read_all_adcs.check()) {
    ...
    mcu_analog_readings.set_steering_2(analogRead(STEERING_2));
    //teensy adc's
    ...
    read_all_adc_channels(&adc_fl, FL_inputs);
    ...
    mcu_potentiometers.set_pot2(FR_inputs[SUS_POT_FR]);
