...
/* queue to store up to 64 messages (aligned to 4-byte boundary) */
K_MSGQ_DEFINE(uart_msgq, sizeof(uart_message), 64, 4);
...
void serial_cb(const struct device *dev, void *user_data)
{
	...
	while (uart_irq_rx_ready(uart_dev)) {
		...
		//if ((c == '\n' || c == '\r') && rx_buf_pos > 0) {
		if ((c == (uint8_t) 0) && rx_buf.length > 0) {
			newlines++;
			if (newlines >= 2)
			{
				/* terminate string */
				rx_buf.buf[rx_buf.length] = '\0';
				...
				rx_buf.buf[rx_buf.length++] = c;
			}
		} else if (rx_buf.length < (sizeof(rx_buf) - 1)) {
			newlines = 0;
			rx_buf.buf[rx_buf.length++] = c;
		}
...
void uart_thread(void) {
	...
	while (1) {
		if (k_msgq_get(&uart_msgq, &tx_buf, K_FOREVER) == 0) {
			print_uart("Echo: Hello");
			//print_uart(tx_buf.buf);
			print_uart("\r\n");
