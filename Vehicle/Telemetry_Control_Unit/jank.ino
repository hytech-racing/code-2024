...
void parse_detailed_voltages(const CAN_message_t& rx_msg) {
  detailed_voltages.load(rx_msg.buf);
  cell_voltages[detailed_voltages.get_ic_id()][(detailed_voltages.get_group_id()*3)] = detailed_voltages.get_voltage_0();
  cell_voltages[detailed_voltages.get_ic_id()][(detailed_voltages.get_group_id()*3)+1] = detailed_voltages.get_voltage_1();
  cell_voltages[detailed_voltages.get_ic_id()][(detailed_voltages.get_group_id()*3)+2] = detailed_voltages.get_voltage_2();
}
...
void print_voltages_temps() {
  ESP.println("---------------------------------------------------------------");
  ESP.print("Total pack voltage: "); ESP.print(voltages.get_total() / 100.0, 4); ESP.print("V\t"); ESP.print("Max voltage differential: "); ESP.print(voltages.get_high() / 10000.0 - voltages.get_low() / 10000.0, 4); ESP.println("V");
  ESP.print("Max V: "); ESP.print(voltages.get_high() / 10000.0, 4); ESP.print("V \t ");
  ESP.print("Min V: "); ESP.print(voltages.get_low() / 10000.0, 4); ESP.print("V \t");
  ESP.print("Avg V: "); ESP.print(voltages.get_average() / 10000.0, 4); ESP.println("V \t");
  ESP.println("Raw Cell Voltages\t\t\t\t\t\t\t\t\t\t\t\t\tBalancing Status");
  ESP.print("\tC0\tC1\tC2\tC3\tC4\tC5\tC6\tC7\tC8\tC9\tC10\tC11\t\t");
  for (int ic = 0; ic < TOTAL_IC; ic++) {
    ESP.print("IC"); ESP.print(ic); ESP.print("\t");
    for (int cell = 0; cell < EVEN_IC_CELLS; cell++) {
      ESP.print(cell_voltages[ic][cell] / 10000.0, 4); ESP.print("V\t");
    }
    ESP.println();
  }
}
