...
void setup()
{
  ...
  myGNSS.setUART1Output(COM_TYPE_UBX);
  myGNSS.setUART1Input(COM_TYPE_UBX | COM_TYPE_NMEA | COM_TYPE_RTCM3);
  */
  ...
  while (myGNSS.begin(Wire) == false) //Connect to the u-blox module using Wire port
  {
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  if (myData.data < 0x10) Serial.print(F("0"));
  Serial.print(myData.data, HEX);
  Serial.print(F(" "));
